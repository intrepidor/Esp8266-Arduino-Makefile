...
# Get MKSPIFFS Tool
cp -np download/mkspiffs-0.1.2-windows.zip .
...
mv mkspiffs-0.1.2-windows.zip download
chmod +x bin/mkspiffs.exe

# Get Xtensa GCC Compiler
cp -np download/win32-xtensa-lx106-elf-gb404fb9-2.tar.gz .
...
mv win32-xtensa-lx106-elf-gb404fb9-2.tar.gz download

# Get ESPTOOL
cp -np download/esptool-0.4.6-win32.zip .
...
mv esptool-0.4.6-win32.zip download
chmod +x bin/esptool.exe

# Get Arduino IDE
cp -p download/$ARDUINO_VER-windows.zip $ARDUINO_VER-windows.zip
...
mv $ARDUINO_VER-windows.zip download
